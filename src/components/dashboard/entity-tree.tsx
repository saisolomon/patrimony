...
function buildEntityTree(entities: SerializedEntity[]): EntityNode[] {
  ...
  for (const entity of entities) {
    ...
    if (entity.parentId) {
      const parentNode = nodeMap.get(entity.parentId);
      if (parentNode) {
...
function EntityCard({
  ...
  const parentName = node.entity.parentId
    ? entities.find((e) => e.id === node.entity.parentId)?.name
    : undefined;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="text-right">
            ...
            <p className="mt-0.5 text-xs text-text-muted">
              {node.entity.assets.length} asset
              {node.entity.assets.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>

        {/* Assets within entity */}
        {node.entity.assets.length > 0 && (
          <div className="mt-4 space-y-1.5">
            <p className="text-[10px] font-medium uppercase tracking-widest text-text-muted">
              Holdings
            </p>
            <div className="grid gap-2 sm:grid-cols-2">
              {node.entity.assets.map((assetName, idx) => (
                <div
                  key={idx}
...
export function EntityTree({ entities }: EntityTreeProps) {
  const entityTree = useMemo(() => buildEntityTree(entities), [entities]);

  const totalValue = entities.reduce((sum, e) => sum + e.totalValue, 0);
    ...
    <>
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-text-muted">
          {entities.length} legal entities managing {formatCurrency(totalValue)}
        </p>
      </div>
